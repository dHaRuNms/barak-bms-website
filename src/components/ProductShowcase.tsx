...
export const ProductShowcase: React.FC = () => {
  ...
  return (
    <section id="ecosystem" className="py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 bg-[#050505]">
      ...
      {/* Product Selector Navigation Tabs (Prime Intellect FIG style) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        ...
        <button
          ...
          className={`p-4 rounded-xl border text-left transition-all duration-200 flex items-center gap-3 ${
            activeProductTab === 'clara'
              ? 'bg-[#0f0f0f] border-cyan-400 text-white shadow-lg'
              : 'bg-[#090909] border-white/[0.08] text-slate-400 hover:text-slate-200 hover:border-white/20'
          }`}
        >
          <div className={`p-2.5 rounded-lg border ${activeProductTab === 'clara' ? 'bg-cyan-950/80 border-cyan-400 text-cyan-300' : 'bg-[#0e0e0e] border-white/10 text-slate-400'}`}>
            <Bot className="w-5 h-5" />
          </div>
          <div>
            <div className="text-[10px] font-mono text-cyan-400 uppercase">FIG. 01</div>
            <div className="text-sm font-sans font-bold text-slate-100">Clara AI Copilot</div>
          </div>
        </button>
        ...
        <button
          ...
          className={`p-4 rounded-xl border text-left transition-all duration-200 flex items-center gap-3 ${
            activeProductTab === 'eda'
              ? 'bg-[#0f0f0f] border-emerald-400 text-white shadow-lg'
              : 'bg-[#090909] border-white/[0.08] text-slate-400 hover:text-slate-200 hover:border-white/20'
          }`}
        >
          <div className={`p-2.5 rounded-lg border ${activeProductTab === 'eda' ? 'bg-emerald-950/80 border-emerald-400 text-emerald-300' : 'bg-[#0e0e0e] border-white/10 text-slate-400'}`}>
            <Code2 className="w-5 h-5" />
          </div>
          <div>
            <div className="text-[10px] font-mono text-emerald-400 uppercase">FIG. 02</div>
            <div className="text-sm font-sans font-bold text-slate-100">Open EDA Suite</div>
          </div>
        </button>
        ...
        >
          <div className={`p-2.5 rounded-lg border ${activeProductTab === 'emulator' ? 'bg-cyan-950/80 border-cyan-400 text-cyan-300' : 'bg-[#0e0e0e] border-white/10 text-slate-400'}`}>
            <Binary className="w-5 h-5" />
          </div>
          <div>
            <div className="text-[10px] font-mono text-cyan-400 uppercase">FIG. 03</div>
            <div className="text-sm font-sans font-bold text-slate-100">Digital Twin</div>
          </div>
        </button>
        ...
        >
          <div className={`p-2.5 rounded-lg border ${activeProductTab === 'fleet' ? 'bg-emerald-950/80 border-emerald-400 text-emerald-300' : 'bg-[#0e0e0e] border-white/10 text-slate-400'}`}>
            <Car className="w-5 h-5" />
          </div>
          <div>
            <div className="text-[10px] font-mono text-emerald-400 uppercase">FIG. 04</div>
            <div className="text-sm font-sans font-bold text-slate-100">Fleet Dispatch</div>
          </div>
        </button>
      ...
      <div className="stark-panel rounded-2xl p-6 sm:p-8 border border-white/10 energy-stream-border">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-5 space-y-4">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded bg-cyan-950 border border-cyan-500/40 text-cyan-400 font-mono text-xs font-semibold">
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-slate-400 text-xs">Rapidly interprets telemetry data, register faults, and pin voltage errors.</span>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle2 className="w-4 h-4 text-cyan-400 shrink-0 mt-1" />
            ...
            <div className="lg:col-span-7 bg-[#070707] rounded-2xl border border-white/10 overflow-hidden flex flex-col h-[460px] shadow-2xl">
              <div className="bg-[#0c0c0c] px-4 py-3 border-b border-white/[0.08] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  >
                    <div 
                      className={`max-w-[85%] p-3.5 rounded-xl ${
                        m.sender === 'user' 
                          ? 'bg-[#121b24] border border-cyan-500/40 text-cyan-200' 
                          : 'bg-[#101010] border border-white/10 text-slate-200'
                      }`}
                    >
                      ...
                      {m.codeSnippet && (
                        <div className="mt-2 p-2 rounded bg-[#050505] border border-white/[0.08] text-[10px] text-cyan-300 font-mono">
                          {m.codeSnippet}
                        </div>
                      )}
                    </div>
              ...
              <div className="px-3 py-2 bg-[#0a0a0a] border-t border-white/[0.06] flex gap-2 overflow-x-auto">
                {samplePrompts.map((p, i) => (
                  <button
                    key={i}
                    onClick={() => handleSendPrompt(p)}
                    className="whitespace-nowrap text-[10px] font-mono px-2.5 py-1 rounded bg-[#121212] hover:bg-cyan-950/60 border border-white/10 hover:border-cyan-400 text-slate-300 hover:text-cyan-300 transition-colors"
                  >
                    + {p}
                  </button>
                ))}
              ...
              <form 
                ...
                className="p-3 bg-[#070707] border-t border-white/[0.08] flex gap-2"
              >
                <input 
                  type="text"
                  value={inputPrompt}
                  ...
                  className="flex-1 bg-[#101010] border border-white/10 rounded-xl px-4 py-2 text-xs font-mono text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                />
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl bg-white text-black hover:bg-slate-200 font-bold text-xs font-mono transition-colors flex items-center gap-1.5"
                >
                  <span>Send</span>
        ...
        {activeProductTab === 'eda' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-6 space-y-4">
              <span className="px-2.5 py-0.5 rounded bg-emerald-950 border border-emerald-500/40 text-emerald-400 font-mono text-xs font-semibold">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                <div className="p-3.5 rounded-xl bg-[#090909] border border-white/[0.08]">
                  <div className="text-xs font-mono text-emerald-400 font-bold mb-1">Developer-Centric</div>
                  <p className="text-xs text-slate-400 font-body">Streamlines schematic capture, PCB layout DRC validation, and unified component footprint libraries.</p>
                </div>
                <div className="p-3.5 rounded-xl bg-[#090909] border border-white/[0.08]">
                  <div className="text-xs font-mono text-emerald-400 font-bold mb-1">Rapid Prototyping</div>
                  <p className="text-xs text-slate-400 font-body">Accelerates hardware design iterations for electronics engineers, startups, and academic researchers.</p>
                </div>
              </div>
            </div>

            <div className="lg:col-span-6 bg-[#070707] p-6 rounded-2xl border border-white/10 font-mono text-xs text-slate-300 space-y-4">
              ...
              <div className="p-3 rounded-lg bg-[#0e0e0e] border border-emerald-500/20 text-slate-300 text-xs">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-6 space-y-4">
              ...
              <div className="space-y-2 pt-2 text-xs font-mono">
                <div className="p-3 rounded-xl bg-[#090909] border border-white/[0.08] flex items-center gap-3">
            ...
            <div className="lg:col-span-6 bg-[#070707] p-6 rounded-2xl border border-white/10">
              <div className="flex justify-between items-center pb-3 border-b border-white/[0.08] text-xs font-mono text-cyan-400">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-6 space-y-4">
              ...
              <div className="space-y-3 pt-2 text-xs">
                <div className="p-3 rounded-xl bg-[#090909] border border-white/[0.08]">
                  <div className="text-xs font-mono text-emerald-400 font-bold mb-1">Real-Time Dispatch & Routing</div>
                  <span className="text-slate-300 font-body">Sub-second rider-driver matching with high-accuracy location tracking and low-latency route calculation.</span>
                </div>
                <div className="p-3 rounded-xl bg-[#090909] border border-white/[0.08]">
                  <div className="text-xs font-mono text-emerald-400 font-bold mb-1">BMS Telemetry Integration</div>
                  <span className="text-slate-300 font-body">Syncs directly with onboard BMS to ensure vehicles are never dispatched below critical range thresholds.</span>
                </div>
              </div>
            </div>

            <div className="lg:col-span-6 bg-[#070707] p-6 rounded-2xl border border-white/10 font-mono text-xs text-slate-300">
              <div className="flex justify-between pb-3 border-b border-white/[0.08] text-emerald-400">
                <span>// DISPATCH & TELEMATICS HUD</span>
                <span>ACTIVE DEPOT: TAMIL NADU</span>
              </div>
              <div className="grid grid-cols-2 gap-3 my-4">
                <div className="p-3 rounded-lg bg-[#0f0f0f] border border-white/[0.06]">
              ...
              <div className="p-3 rounded-lg bg-[#0f0f0f] border border-white/[0.06] space-y-1.5 text-[11px]">
